...
| File | What it is |
|---|---|
| `pso-antispam-X.Y.Z.crate` | The byte-identical .crate uploaded to crates.io. |
| `pso-antispam-X.Y.Z.crate.sig` | cosign blob signature (raw, base64). |
| `pso-antispam-X.Y.Z.crate.pem` | Fulcio-issued ephemeral signing cert (PEM). |
...
- A compromise of `github.com/psonet/pso-antispam` itself (an attacker with push access to `main` can edit the workflow to remove or weaken signing).
...
```sh
REPO=psonet/pso-antispam
TAG=v0.2.2  # or any release ≥ the cutoff
ARTIFACT=pso-antispam-${TAG#v}.crate
...
cosign verify-blob \
  ...
  --certificate-identity-regexp \
    '^https://github\.com/psonet/pso-antispam/\.github/workflows/ci\.yml@refs/(heads/main|tags/v[0-9]+\.[0-9]+\.[0-9]+)$' \
...
For security issues in `pso-antispam` itself (not the signing pipeline), open a [private security advisory](https://github.com/psonet/pso-antispam/security/advisories/new) on GitHub. Do not file a public issue.
